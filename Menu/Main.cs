    ...
    public class Main : MonoBehaviour
    {
        ...
        private void Update()
        {
            try
            {
                ...
                const bool keyRequested = false;

                if (activeMenu == null)
                {
                    if (openRequested || keyRequested)
                    {
                        BuildMenu();
                        PositionMenu(rightHanded, keyRequested);
                        if (handPointer == null)
                        {
        ...
        public static void BuildMenu()
        {
            ...
            titleText.text = $"{Constants.PluginName} <color=grey>[</color><color=white>{activePage + 1}</color><color=grey>]</color>";
        ...
        public static void BuildButton(float offset, ButtonInfo info)
        {
            ...
            textTrans.localPosition = new Vector3(0.064f, 0f, 0.111f - offset / 2.6f);
        ...
        public static int GetCollisionMask()
        {
            cachedLayerMask ??= ~(
            ...
            return cachedLayerMask ?? GTPlayer.Instance.locomotionEnabledLayers;
        }
    }